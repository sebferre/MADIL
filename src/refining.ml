
open Madil_common

let debug = ref false (* set to true to return all candidate refinements with causes for not being valid *)

exception No_local_parse
exception Failed_postprocessing
exception Alt_not_allowed of int (* support *)
exception Alt_cannot_be_pruned
exception Cond_expr_not_valid

(* computing model refinements from parsing data *)

type ('typ,'value,'distrib,'constr,'var,'func) read = ('typ,'value,'distrib,'constr,'var,'func) Model.read * ('value,'distrib,'constr) Data.data
   
let map_reads (f : 'read -> 'data -> 'data) (reads : ('readdata list * bool) list) : ('readdata list * bool) list  =
  List.map
    (fun (example_reads, unselected_reads) ->
      List.map
        (fun (read,data) ->
          let data' = f read data in
          (read, data'))
        example_reads,
      unselected_reads)
    reads
  
let filter_map_reads (f : 'read -> 'data -> 'data option) (selected_reads : ('readdata list * bool) list) : ('readdata list * bool) list =
  List.filter_map
    (fun (example_reads, unselected_reads) ->
      let defined_example_reads, undefined_reads =
        List.partition_map
          (fun (read,data) ->
            match f read data with
            | None -> Either.Right ()
            | Some data' -> Either.Left (read, data'))
          example_reads in
      if defined_example_reads = []
      then None
      else Some (defined_example_reads, unselected_reads || undefined_reads <> []))
    selected_reads

type ('typ,'value,'distrib,'constr,'var,'func) best_read = (* should be named best_read *)
  { unselected_reads : bool; (* flag for out-of-branch alt reads *)
    matching : bool; (* matching flag *)
    read : ('typ,'value,'distrib,'constr,'var,'func) Model.read; (* the selected best read, first one when matching=false *)
    data : ('value,'distrib,'constr) Data.data;
    new_data : ('value,'distrib,'constr) Data.data; (* the new data when matching=true, the old data when matching=false *)
  }

let best_reads_stats (best_reads : ('typ,'value,'distrib,'constr,'var,'func) best_read list) : int * int = (* support, total *)
  List.fold_left
    (fun (supp,nb) best_read ->
      if best_read.matching then supp+1, nb+1 (* positive *)
      else if best_read.unselected_reads then supp, nb (* out of scope *)
      else supp, nb+1) (* negative *)
    (0,0) best_reads

let inter_union_reads
      (get_rs : (('typ,'value,'distrib,'constr,'var,'func) read as 'read) -> ('ref * 'data result) list)
      (reads : ('readdata list * bool) list)
    : ('ref, ('typ,'value,'distrib,'constr,'var,'func) best_read list result) Mymap.t =
  (* given a function extracting refinements (submodels) from each read,
     return a set of such refinements, each mapped to the dl-shortest reads supporting it, along with new data *)
  let process_example reads unselected_reads =
    Common.prof "Refining.inter_union_reads/process_example" (fun () ->
    assert (reads <> []);
    let read0, data0 = List.hd reads in
    let alt_read = {unselected_reads; matching = false; read = read0; data = data0; new_data = data0} in
    let refs =
      List.fold_left
        (fun refs (read,data) ->
          let refs_read =
            Common.prof "Refining.inter_union_reads/process_example/get_rs" (fun () ->
            get_rs (read,data)) in
          List.fold_left (* union(refs, refs_read) *)
            (fun refs (r,new_data_res) ->
              match Mymap.find_opt r refs with
              | Some best_read_res ->
                 (match best_read_res, new_data_res with
                  | Result.Error _, Result.Ok new_data ->
                     let new_best_read =
                       {unselected_reads; matching = true; read; data; new_data} in
                     Mymap.add r (Result.Ok new_best_read) refs
                  | _ -> refs)
              | None ->
                 let best_read_res =
                   let| new_data = new_data_res in
                   Result.Ok {unselected_reads; matching = true; read; data; new_data} in
                 Mymap.add r best_read_res refs)
            refs refs_read)
        Mymap.empty reads in
    alt_read, refs)
  in
  Common.prof "Refining.inter_union_reads" (fun () ->
  match List.rev reads with (* rev to have best_reads in right order at the end *)
  | [] -> assert false
  | (example0_reads, example0_unselected_reads) :: other_reads ->
     let alt_read0, refs0 = process_example example0_reads example0_unselected_reads in
     let alt_reads = [alt_read0] in
     let refs =
       refs0
       |> Mymap.map
            (fun best_read0 ->
              match best_read0 with
              | Result.Ok best_read0 -> [best_read0], []
              | Result.Error err -> [alt_read0], [err]) in
     let alt_reads, refs = (* TODO: is alt_read(s) necessary here? *)
       List.fold_left
         (fun (alt_reads,refs) (exampleI_reads, exampleI_unselected_reads) ->
           let alt_readI, refsI = process_example exampleI_reads exampleI_unselected_reads in
           let refs =
             Common.prof "Refining.inter_union_reads/merge" (fun () ->
             Mymap.merge (* intersection(refs, refsI) *)
               (fun r best_reads_opt best_readI_opt ->
                 match best_reads_opt, best_readI_opt with
                 | Some (best_reads, errs), Some best_readI ->
                    (match best_readI with
                     | Result.Ok best_readI ->
                        Some (best_readI :: best_reads, errs)
                     | Result.Error errI ->
                        Some (alt_readI :: best_reads, errI :: errs))
                 | Some (best_reads, errs), None ->
                    Some (alt_readI :: best_reads, errs)
                 | None, Some best_readI ->
                    (match best_readI with
                     | Result.Ok best_readI ->
                        Some (best_readI :: alt_reads, [No_local_parse])
                     | Result.Error errI ->
                        Some (alt_readI :: alt_reads, [errI; No_local_parse]))
                 | _ -> assert false (* None *))
               refs refsI) in
           let alt_reads = alt_readI :: alt_reads in
           alt_reads, refs)
         (alt_reads, refs) other_reads in
     Mymap.map
       (fun (best_reads, errs) ->
         if List.for_all (fun err -> err = No_local_parse) errs (* no error or only parse failures *)
         then (Result.Ok best_reads)
         else (Result.Error (List.hd errs))) (* unexpected error *)
       refs)

let extend_partial_best_reads
      (selected_reads : ((('typ,'value,'distrib,'constr,'var,'func) read as 'read) list * bool) list)
      (best_reads : (('typ,'value,'distrib,'constr,'var,'func) best_read as 'best_read) list)
      (check_alt_read : 'read -> ('read * 'data) option)
    : 'best_read list =
  List.map2
    (fun (reads, unselected_reads) best_read ->
      if best_read.matching
      then best_read (* already matches some refinement *)
      else
        (match List.find_map check_alt_read reads with
         | Some ((read,data), new_data) ->
            {unselected_reads = best_read.unselected_reads; matching = true;
             read; data; new_data} (* new match *)
         | None -> best_read)) (* no change *)
    selected_reads best_reads

let make_alt_if_allowed_and_needed
      ~(var0 : 'var)
      ~(allowed : bool) ~(nb : int) ~(supp : int)
      (m_true : (('typ,'value,'var,'const,'func) Model.model as 'model))
      (m_false : 'model)
      (best_reads : (('typ,'value,'distrib,'constr,'var,'func) best_read as 'best_read) list)
    : ('model * 'best_read list result) Myseq.t =
  if supp = nb then (* no need for alternative *)
    Myseq.return (m_true, Result.Ok best_reads)
  else if allowed then
    (* making an alternative, model and data *)
    let prob = float supp /. float nb in
    let m' = Model.Alt (var0, Undet prob, m_true, m_false) in
    let best_reads' =
      List.map
        (fun {unselected_reads; matching; read; data; new_data} ->
          let prob = if matching then prob else 1. -. prob in
          let new_data = Data.DAlt (prob, matching, new_data) in
          {unselected_reads; matching; read; data; new_data})
        best_reads in
    Myseq.return (m', Result.Ok best_reads')
  else (* alt not allowed *)
    if !debug
    then Myseq.return (m_true, Result.Error (Alt_not_allowed supp))
    else Myseq.empty

type ('typ,'value,'distrib,'var,'constr,'func) refiner =
  include_expr:bool ->
  nb_env_vars:int ->
  env_vars:('var,'typ) Expr.binding_vars ->
  dl_M:dl -> (* current model DL *)
  (* NOTE: dl_M does not matter for ranking because an invariant of parsing and refinement *)
  (('typ,'value,'var,'constr,'func) Model.model as 'model) ->
  'var Myseq.t -> (* fresh variables viz the model *)
  ('typ,'value,'distrib,'constr,'var,'func) Model.read list list
  -> (('var,'constr) Model.path (* refinement location *)
      * 'model (* refined submodel *)
      * int (* support *)
      * dl result (* new DL or error *)
      * 'model (* new model, after applying refinement *)
      * 'var Myseq.t (* remaining fresh vars *)
     ) Myseq.t
  (* result: a sequence of path-wise refinements with support and estimate DL *)

let refinements
      ~(pruning : bool)
      ~(xp_value : 'value html_xp)
      ~(xp_model : 'model html_xp)
      ~(alpha : float)
      ~(max_expr_size : int)
      ~(max_expr_refinements_per_read : int)
      ~(max_expr_refinements_per_var : int)
      (* DEPR ~(max_steps : int) *)
      ~(max_refinements : int)
      ~(asd : ('typ,'abs_typ) Model.asd)
      ~(typ_bool : 'typ)
      ~(value_of_bool : bool -> 'value)
      ~(var0 : 'var)
      ~(dl_model : nb_env_vars:int -> (('typ,'value,'var,'constr,'func) Model.model as 'model) -> dl)
      ~(dl_data : (('value,'distrib,'constr) Data.data as 'data) -> dl)
      ~(parse_bests : 'model -> ('distrib,'var,'typ,'value,'constr) Model.parse_bests)
      ~(make_index : ('var,'typ,'value) Expr.bindings -> ('typ,'value,'var,'func) Expr.index)      
      (* DEPR      ~(decompositions : 'typ -> 'value list list -> 'model list) *)
      ~(refinements_value : 'typ -> 'value -> 'model list)
      ~(refinements_any : 'typ -> 'value -> 'model list)
      ~(refinements_pat : 'typ -> 'constr -> 'model array -> 'value -> 'model list) (* refined submodel with remaining fresh vars *)
      ~(refinements_pat_expr : env_vars:('var,'typ) Expr.binding_vars -> 'typ -> 'value -> 'model list)
      ~(postprocessing : 'typ -> 'model -> 'model -> supp:int -> nb:int -> alt:bool -> 'best_read list
                         -> ('model * 'best_read list) Myseq.t) (* converting refined submodel, alt mode (true if partial match), support, and best reads to a new model and corresponding new data *)
    : ('typ,'value,'distrib,'var,'constr,'func) refiner =

  let parse_best m bindings v r = Common.prof "Refining.refinements/parse_best" (fun () ->
    try
      match parse_bests m bindings v r with
      | Result.Ok ((d',dl')::_) -> Result.Ok d'
      | _ -> Result.Error No_local_parse
    with exn ->
      if !debug
      then Result.Error exn (* to assign unexpected errors to individual refinements, for easier debugging *)
      else raise exn) (* otherwise, make those errors silent *)
  in
  fun ~include_expr ~nb_env_vars ~env_vars ~dl_M m0 varseq0 reads ->
  Myseq.prof "Refining.refinements" (
  let env_vars = if include_expr then env_vars else Expr.binding_vars0 in (* hide vars to patterns *)
  let aux_dl_new dl_m m_new best_reads =
    let dl_m_new = dl_model ~nb_env_vars m_new in
    let dl_new =
      dl_M -. dl_m +. dl_m_new
      +. alpha *. Mdl.sum best_reads
                    (fun {matching; read; data; new_data} ->
                      if matching
                      then
                        let dl_d = dl_data data in
                        let dl_d_new = dl_data new_data in
                        read.dl -. dl_d +. dl_d_new
                      else read.dl) (* no change in this case *) in
    dl_round dl_new (* rounding to absorb float error accumulation *)
  in
  let aux_gen (type r)
        ctx m selected_reads (* ctx is reverse path *)
        (read_refs : 'read * 'data -> (r * 'data result) list)
        (model_of_ref : r -> 'model)
        (postprocessing : 'model -> supp:int -> nb:int -> alt:bool -> 'best_reads -> ('model * 'best_reads result) Myseq.t)
      : ('path * 'model * int * dl result) Myseq.t =
    Myseq.prof "Refining.refinements/aux_gen" (
    let p = List.rev ctx in (* local path *)
    let dl_m = dl_model ~nb_env_vars m in
    let selected_reads = (* local re-parsing of data from value for comparability with m' *)
      Common.prof "Refining.refinements/aux_gen/reparsing" (fun () ->
      map_reads
        (fun read data ->
          let v, r = Data.value_distrib data in
          match parse_best m read.Model.bindings v r with
          | Result.Ok data -> data
          | Result.Error exn -> (* should not happen *)
             print_endline "ERROR failed local parsing with current model";
             print_string "local model: "; pp_endline xp_model m;
             print_string "parsed value: "; pp_endline xp_value v;
             print_endline (Printexc.to_string exn);
             data)
        selected_reads) in
    let r_best_reads = inter_union_reads read_refs selected_reads in
    let* r, best_reads_res = Mymap.to_seq r_best_reads in
    let m_new = model_of_ref r in
    match best_reads_res with
    | Result.Error error ->
       if !debug
       then Myseq.return (p, m_new, 0, Result.Error error)
       else Myseq.empty
    | Result.Ok best_reads ->
       let supp, nb = best_reads_stats best_reads in
       if supp = 0 then (* only alt_reads, no parse at all *)
         if !debug
         then Myseq.return (p, m_new, supp, Result.Error No_local_parse)
         else Myseq.empty
       else
         let alt = (supp < nb) in
         let* m_new, best_reads_res =
           postprocessing m_new ~supp ~nb ~alt best_reads in
         match best_reads_res with
         | Result.Error error ->
            if !debug
            then Myseq.return (p, m_new, supp, Result.Error error)
            else Myseq.empty
         | Result.Ok best_reads ->
            let dl_new = aux_dl_new dl_m m_new best_reads in
            Myseq.return (p, m_new, supp, Result.Ok dl_new))
  in
  let rec aux ctx m selected_reads =
  Myseq.prof "Refining.refinements/aux" (
  (* DEPR if steps >= max_steps then Myseq.empty else *)
  if selected_reads = [] then Myseq.empty
  else
    match m with
    | Model.Def (x,m1) ->
       let ctx = [Model.Alias (x,ctx)] in
       aux ctx m1 selected_reads
    | Model.Any t ->
       Myseq.interleave
         [aux_const ctx m selected_reads;
          aux_expr ctx m selected_reads;
          aux_any_pat ctx m t (refinements_any t) selected_reads;
          aux_pat_expr ~env_vars ctx m t selected_reads]
         (* DEPR aux_decomp ~steps ctx t m selected_reads] *)
    | Model.Pat (t,c,src,args) ->
       Myseq.interleave
         (aux_const ctx m selected_reads
          :: aux_expr ctx m selected_reads
          :: aux_any_pat ctx m t (refinements_pat t c args) selected_reads
          (* XX :: aux_pat_expr ~env_vars ctx m t selected_reads *) (* pb: can erase good motif irreversably *)
          :: Array.to_list
              (Array.mapi
                 (fun i mi ->
                   let ctxi = Model.Field (c,i)::ctx in
                   aux ctxi mi
                     (map_reads
                        (fun read -> function
                         | Data.DPat (_v, _r, _c, _vsrc, args) ->
                            assert (i >= 0 && i < Array.length args);
                            args.(i)
                         | _ -> assert false)
                        selected_reads))
                 args))
         
    | Model.Alt (xc,c,m1,m2) ->
       Myseq.interleave
         [ aux_const ctx m selected_reads;
           aux_expr ctx m selected_reads;
       
           (match c with
            | Undet _ ->
               if pruning
               then aux_alt_prune ctx m m1 m2 selected_reads
               else aux_alt_cond_undet ctx m xc c m1 m2 selected_reads
            | BoolExpr _ -> Myseq.empty);
           
           (let ctx1 = Model.Branch true :: ctx in
            let sel1 =
              filter_map_reads
                (fun read -> function
                 | Data.DAlt (prob,b,d12) ->
                    if b
                    then Some d12
                    else None
                 | _ -> assert false)
                selected_reads in
            aux ctx1 m1 sel1);
                        
           (let ctx2 = Model.Branch false :: ctx in
            let sel2 =
              filter_map_reads
                (fun read -> function
                 | Data.DAlt (prob,b,d12) ->
                    if not b
                    then Some d12
                    else None
                 | _ -> assert false)
                selected_reads in
            aux ctx2 m2 sel2) ]        
    | Model.Expr (Expr.Const (t,v)) ->
       if pruning (* pruning constants *)
       then
         aux_gen ctx m selected_reads
           (fun (read, data : _ read) ->
             refinements_value t v
             |> List.map (fun m' -> (m', Result.Ok data)))
           (fun m' -> m')
           (fun m' ~supp ~nb ~alt best_reads ->
             Myseq.return (m', Result.Ok best_reads))
       else (*Myseq.empty*)
         (* generalizing constant into variable expression *)
         aux_expr ctx m selected_reads
         |> Myseq.filter
              (fun (p,m',supp,dl') ->
                match m' with
                | Model.Expr (Expr.Const _) -> false
                | _ -> true)
    | Model.Expr e -> Myseq.empty
    | Model.Derived t -> Myseq.empty)
(* DEPR  and aux_decomp ~steps ctx t m selected_reads =
    if steps+1 >= max_steps then Myseq.empty (* because aux returns nothing at max_steps *)
    else
    let p = List.rev ctx in
    let dl_m = dl_model ~nb_env_vars m in
    let nb = List.length selected_reads in
    let vss =
      List.map
        (fun (example,_) ->
          List.map
            (fun (read,data) ->
              Data.value data)
            example)
        selected_reads in
    let refs =
      decompositions t vss
      |> List.fold_left
           (fun refs m' ->
             let selected_reads' =
               filter_map_reads
                 (fun read data ->
                   let v, r = Data.value_distrib data in
                   (*let data_res = parse_best m read.Model.bindings input in*)
                   let data'_res = parse_best m' read.Model.bindings v r in
                   (* (match data_res, data'_res with (* checking data DL invariance *)
                    | Result.Ok data, Result.Ok data' ->
                       let dl = dl_data data in
                       let dl' = dl_data data' in
                       if dl_round dl <> dl_round dl'
                       then Printf.printf "ERROR decomposition data DL: %.5f vs %.5f\n" dl dl' 
                       else print_endline "OK decomposition data DL"
                    | _ -> ()); *)
                   Result.to_option data'_res)
                 selected_reads in
             let supp = List.length selected_reads' in
             if supp = nb
             then (m', Result.Ok selected_reads')::refs
             else
               if !debug
               then (m', Result.Error No_local_parse)::refs
               else refs)
           [] in
    Myseq.interleave
      (List.map
         (fun (m', selected_reads'_res) ->
    match m', selected_reads'_res with
    | Model.Pat (_t, c, src, args), Result.Ok selected_reads' ->
       let dl_m' = dl_model ~nb_env_vars m' in
       let n = Array.length args in
       Myseq.interleave
         (Array.to_list
            (Array.mapi
               (fun i arg_i ->
                 let ctx_i = Model.Field (c,i)::ctx in
                 let ctx_i, m_i =
                   match arg_i with
                   | Model.Def (x,m_i) -> [Model.Alias (x,ctx_i)], m_i
                   | _ -> ctx_i, arg_i in
                 let selected_reads_i =
                   map_reads
                     (fun read -> function
                      | Data.DPat (_v, _r, _c, _src, dargs) ->
                         assert (Array.length dargs = n);
                         dargs.(i)
                      | _ -> assert false)
                     selected_reads' in
                 let* p_i, m'_i,  supp, dl'_i_res =
                   aux ~steps:(steps+1) ctx_i m_i selected_reads_i in
                 let m'_i =
                   match arg_i with
                   | Model.Def (x,_) -> Model.Def (x,m'_i)
                   | _ -> m'_i in
                 let m' =
                   let args' = Array.copy args in
                   args'.(i) <- m'_i;
                   Model.make_pat t c ~src args' in
                 let dl'_res = (* adding extra-cost of decomposition on model, assuming no cost on data *)
                   let| dl'_i = dl'_i_res in
                   Result.Ok (dl'_i -. dl_m +. dl_m') in
                 Myseq.return (p, m', supp, dl'_res)) (* pattern c(args) is assumed DL-invariant *)
               args))
    | _, Result.Error error ->
       if !debug
       then Myseq.return (p, m', 0, Result.Error error)
       else Myseq.empty
    | _ ->
       if !debug
       then Myseq.return (p, m', 0, Result.Error (Failure "decompositions must be patterns"))
       else Myseq.empty)
         refs) *)
  and aux_const ctx m selected_reads = (* QUICK *)
    let t = Model.typ m in
    if pruning || not (asd#expr_opt t) then Myseq.empty
    else
       let t = Model.typ m in
       let allowed = asd#alt_opt t in
       aux_gen ctx m selected_reads
         (fun (read, data : _ read) ->
           Common.prof "Refining.refinements/aux_const/get_rs" (fun () ->
           let v, r = Data.value_distrib data in
           let e = Expr.Const (t,v) in
           let me = Model.make_expr e in
           let data' = Data.make_dexpr v r in
           [(me, Result.Ok data')]))
         (fun m' -> m')
         (fun m' ~supp ~nb ~alt best_reads ->
           Myseq.if_not_empty_else
             (let* m', best_reads = postprocessing t m m' ~supp ~nb ~alt best_reads in
              make_alt_if_allowed_and_needed ~var0
                ~allowed ~supp ~nb
                m' m best_reads)
             (if !debug
              then Myseq.return (m', Result.Error Failed_postprocessing)
              else Myseq.empty))
       |> Myseq.sort (fun (p1,r1,supp1,dl1_res) (p2,r2,supp2,dl2_res) ->
              let c = Stdlib.compare supp2 supp1 in (* higher support first *)
              if c = 0
              then
                match dl1_res, dl2_res with
                | Result.Ok dl1, Result.Ok dl2 ->
                   let c1 = dl_compare dl1 dl2 in
                   if c1 = 0
                   then Stdlib.compare (p1,r1) (p2,r2)
                   else c1
                | Result.Ok _, _ -> -1
                | _, Result.Ok _ -> 1
                | _ -> 0
              else c)
  and aux_expr ctx m selected_reads = (* QUICK *)
    let t = Model.typ m in
    if pruning || not include_expr || not (asd#expr_opt t) then Myseq.empty
    else
       let allowed = asd#alt_opt t in
       aux_gen ctx m selected_reads
         (fun (read, data : _ read) -> Common.prof "Refining.refinements/aux_expr/get_rs" (fun () ->
           let v, r = Data.value_distrib data in
           let s_expr = (* index expressions evaluating to v *)
             let index = Model.force_index ~make_index read in
             let es = index#lookup (t,v) in
             Expr.Exprset.to_seq ~max_expr_size es in
           Common.prof "Refining.refinements/aux_expr/get_rs/enum" (fun () ->
           s_expr
           |> Myseq.slice ~offset:0 ~limit:max_expr_refinements_per_read
           |> Myseq.fold_left
                (fun refs e ->
                  let me = Model.make_expr e in
                  let data' = Data.make_dexpr v r in
                  (me, Result.Ok data')::refs)
                [])))
         (fun m' -> m')
         (fun m' ~supp ~nb ~alt best_reads ->
           Myseq.if_not_empty_else
             (let* m', best_reads = postprocessing t m m' ~supp ~nb ~alt best_reads in
              make_alt_if_allowed_and_needed ~var0
                ~allowed ~supp ~nb
                m' m best_reads)
             (if !debug
              then Myseq.return (m', Result.Error Failed_postprocessing)
              else Myseq.empty))
       |> Myseq.sort (fun (p1,r1,supp1,dl1_res) (p2,r2,supp2,dl2_res) ->
              let c = Stdlib.compare supp2 supp1 in (* higher support first *)
              if c = 0
              then
                match dl1_res, dl2_res with
                | Result.Ok dl1, Result.Ok dl2 ->
                   let c1 = dl_compare dl1 dl2 in
                   if c1 = 0
                   then Stdlib.compare (p1,r1) (p2,r2)
                   else c1
                | Result.Ok _, _ -> -1
                | _, Result.Ok _ -> 1
                | _ -> 0
              else c)
       |> Myseq.slice ~limit:max_expr_refinements_per_var
  and aux_any_pat ctx m t get_refs selected_reads =
    let allowed = asd#alt_opt t in
    aux_gen ctx m selected_reads
      (fun (read, data : _ read) -> Common.prof "Refining.refinements/aux_any/get_rs" (fun () ->
        let v, r = Data.value_distrib data in
        get_refs v
        |> List.fold_left
             (fun refs m' ->
               let data'_res = parse_best m' read.bindings v r in
               match data'_res with
               | Result.Ok data' ->
                  (m',data'_res)::refs (* could add what follows as an alternative *)
               | Result.Error error -> (* does not match new model *)
                  if !debug
                  then (m',data'_res)::refs
                  else refs)
             []))
      (fun m' -> m')
      (fun m' ~supp ~nb ~alt best_reads ->
        Myseq.if_not_empty_else
          (let* m', best_reads =
             postprocessing t m m' ~supp ~nb ~alt best_reads in
           make_alt_if_allowed_and_needed ~var0
             ~allowed ~supp ~nb
             m' m best_reads)
          (if !debug
           then Myseq.return (m', Result.Error Failed_postprocessing)
           else Myseq.empty))
  and aux_pat_expr ~env_vars ctx m t selected_reads =
    if not include_expr then Myseq.empty
    else aux_any_pat ctx m t (refinements_pat_expr ~env_vars t) selected_reads
  and aux_alt_prune ctx m m1 m2 selected_reads =
    aux_gen ctx m selected_reads
      (fun (read, data : _ read) ->
        match data with
        | Data.DAlt (prob, b, d12) ->
           if b
           then [m1, Result.Ok d12]
           else [m2, Result.Ok d12]
        | _ -> assert false)
      (fun m' -> m')
      (fun m' ~supp ~nb ~alt best_reads ->
        if supp = nb
        then Myseq.return (m', Result.Ok best_reads)
        else
          if !debug
          then Myseq.return (m', Result.Error Alt_cannot_be_pruned)
          else Myseq.empty)
  and aux_alt_cond_undet ctx m xc c m1 m2 selected_reads =
    if include_expr
    then
    aux_gen ctx m selected_reads
      (fun (read, data : _ read) ->
        match data with
        | Data.DAlt (_prob, b, d12) ->
           let vc = value_of_bool b in
           let index = Model.force_index ~make_index read in
           let es : _ Expr.Exprset.t = index#lookup (typ_bool, vc) in
           let new_data = Data.DAlt (1.,b,d12) in
           Myseq.fold_left
             (fun rs e -> (e, Result.Ok new_data) :: rs)
             [] (Expr.Exprset.to_seq ~max_expr_size es)
        | _ -> assert false)
      (fun e ->
        let c_new = Model.BoolExpr e in
        Model.Alt (xc, c_new, m1, m2))
      (fun m' ~supp ~nb ~alt best_reads ->
        if supp = nb
        then Myseq.return (m', Result.Ok best_reads)
        else
          if !debug
          then Myseq.return (m', Result.Error Cond_expr_not_valid)
          else Myseq.empty)
    else Myseq.empty
  in
  let selected_reads =
    (* the flag for each example indicates whether there are other reads, used with Alt *)
    List.map
      (fun example_reads ->
        let example_reads =
          List.map
            (fun (read : _ Model.read) ->
              (read, read.data))
            example_reads in
        (example_reads, false))
      reads in
  let* p, r, supp, dl'_res =
    aux (* DEPR ~steps:0 *) Model.ctx0 m0 selected_reads
    |> Myseq.sort (fun (p1,r1,supp1,dl1_res) (p2,r2,supp2,dl2_res) ->
           match dl1_res, dl2_res with
           | Result.Ok dl1, Result.Ok dl2 ->
              let c1 = dl_compare dl1 dl2 in
              if c1 = 0
              then Stdlib.compare (p1,r1) (p2,r2)
              else c1
           | Result.Ok _, _ -> -1
           | _, Result.Ok _ -> 1
           | _ -> 0)
    |> Myseq.slice ~limit:max_refinements in
  let m', varseq' = Model.refine p r m0 varseq0 in
  Myseq.return (p, r, supp, dl'_res, m', varseq'))


let task_refinements
      ~(alpha : float)
      ~(binding_vars : ('typ,'value,'var,'constr,'func) Model.model -> ('var,'typ) Expr.binding_vars)
      ~(input_refinements : ('typ,'value,'distrib,'var,'constr,'func) refiner)
      ~(output_refinements : ('typ,'value,'distrib,'var,'constr,'func) refiner)

      ~(include_expr : bool)
      ~(include_input : bool)
      ~(include_output : bool)
      (m : (('typ,'value,'var,'constr,'func) Task_model.task_model) as 'task_model)
      (prs : ('typ,'value,'distrib,'constr,'var,'func) Task_model.pairs_reads)
      (dsri : ('typ,'value,'distrib,'constr,'var,'func) Task_model.reads)
      (dsro : ('typ,'value,'distrib,'constr,'var,'func) Task_model.reads)
    : (('typ,'value,'var,'constr,'func) Task_model.refinement * ('task_model * dl) result) Myseq.t = (* QUICK Myseq.next *)
  let l = Task_model.dl_model_data ~alpha prs in
  let dl0 = prs.dl0 in
  let dl = l.md in
  Myseq.interleave (* TODO: rather order by estimated dl *)
    [ (if include_input
       then
         let* p, ri, suppi, dli'_res, mi', varseq =
           input_refinements ~include_expr:false ~nb_env_vars:0 ~env_vars:Expr.binding_vars0 ~dl_M:prs.dl_m.i
             m.input_model m.varseq dsri.reads in
         let m_ndl'_res =
           let| dli' = dli'_res in
           let ndl' = dli' /. dl0.i +. dl.o /. dl0.o in (* output not changed, DL normalized *)
           Result.Ok (Task_model.make ~binding_vars varseq mi' m.output_model, ndl') in
         Myseq.return (Task_model.RStep (`Input,p,ri,suppi), m_ndl'_res)
       else Myseq.empty);

      (if include_output
       then
         let* p, ro, suppo, dlo'_res, mo', varseq =
           output_refinements ~include_expr ~nb_env_vars:m.nb_env_vars ~env_vars:m.env_vars ~dl_M:prs.dl_m.o
             m.output_model m.varseq dsro.reads in
         let m_ndl'_res =
           let| dlo' = dlo'_res in
           let ndl' = dl.i /. dl0.i +. dlo' /. dl0.o in (* input not changed, normalized DL *)
           Result.Ok (Task_model.make ~binding_vars varseq m.input_model mo', ndl') in
         Myseq.return (Task_model.RStep (`Output,p,ro,suppo), m_ndl'_res)
       else Myseq.empty) ]

let task_prunings
      ~(alpha : float)
      ~(binding_vars : ('typ,'value,'var,'constr,'func) Model.model -> ('var,'typ) Expr.binding_vars)
      ~(input_prunings : ('typ,'value,'distrib,'var,'constr,'func) refiner)

      (m : (('typ,'value,'var,'constr,'func) Task_model.task_model as 'task_model))
      (prs : ('typ,'value,'distrib,'constr,'var,'func) Task_model.pairs_reads)
      (dsri : ('typ,'value,'distrib,'constr,'var,'func) Task_model.reads)
    : (('typ,'value,'var,'constr,'func) Task_model.refinement * ('task_model * dl) result) Myseq.t = (* QUICK Myseq.next *)
  let l = Task_model.dl_model_data ~alpha prs in
  let dl0 = prs.dl0 in
  let dl = l.md in
  let* pi, ri, suppi, dli'_res, mi', varseq =
    input_prunings ~include_expr:false ~nb_env_vars:0 ~env_vars:Expr.binding_vars0 ~dl_M:dsri.dl_m
      m.input_model m.varseq dsri.reads in
  let m_ndl'_res =
    let| dli' = dli'_res in
    let ndl' = dli' /. dl0.i +. dl.o /. dl0.o in (* output not changed, normalized DL *)
    Result.Ok (Task_model.make ~binding_vars varseq mi' m.output_model, ndl') in
  Myseq.return (Task_model.RStep (`Input,pi,ri,suppi), m_ndl'_res)
